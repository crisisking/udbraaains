...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for location in orm.Location.objects.exclude(player=None):
            report = location.report_set.order_by('-id')[0]
            report.players = location.player_set.all()
            location.player_set.clear()
        


    def backwards(self, orm):
        raise RuntimeError("Can't roll back this migration.")
